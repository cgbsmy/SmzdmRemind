    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        if (hItem != INVALID_HANDLE_VALUE)
        {
			...
			while (TRUE)
            {
                ...
                {
                    int p = si.fPrice * 100;
                    wsprintf(sz, L"<td><a target=\"_blank\" href=\"%s\"><img src=\"https://%s\" width=\"200\" height=\"200\"></a><a target=\"_blank\" href=\"%s\"><button>直达链接</button><br/></a><small><b><p style=\"float:left;text-align:left;color:red\">%d.%2.2d元</b><br/><br/>值%d <span style=\"color:#000000\">值%d</span> 评%d</p><p style=\"text-align:right\">%2.2d-%2.2d %2.2d:%2.2d<br/><br/>%s</p><b><p>%s</p></b></small><font size=\"1\"><div style=\"text-align:left;color:#383838\">",
                        si.szLink,  si.szImg, si.szGoPath, p / 100, p % 100, si.lZhi,si.lBuZhi,si.lTalk, si.st.wMonth, si.st.wDay, si.st.wHour, si.st.wMinute,si.szBusiness, si.szTitle);
